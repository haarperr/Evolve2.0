...
AddEventHandler("ev-buffs:stamina", function(pEnabled, pPercent)
...
AddEventHandler("ev-buffs:setJobLuckMultiplier", function(pJobLuckMultiplier)
  jobLuckMultiplier = 1 + pJobLuckMultiplier + 0.0
  TriggerServerEvent("ev-buffs:setBuffValue", "jobluck", jobLuckMultiplier)
end)
...
AddEventHandler("ev-buffs:setAlertLevelMultiplier", function(pAlertLevelMultiplier)
...
AddEventHandler("ev-buffs:inDoubleBuffZone", function(pBool)
  ...
  Citizen.CreateThread(function()
    ...
    while isSitting and inRestaurantBuffZone do
      if minuteCount >= 5 then
        ...
        TriggerEvent("ev-buffs:applyBuff", "restaurant_double", {{ buff = "double", percent = percent }})
      end
      Citizen.Wait(60000)
