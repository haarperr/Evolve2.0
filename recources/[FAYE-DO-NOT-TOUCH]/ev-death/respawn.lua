...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsEntityDead(player) then
            ...
            local isFromBeatdown = exports["ev-police"]:getIsInBeatmode()

            if isFromBeatdown then
                exports["ev-police"]:setIsInBeatmode(false)
                exports["ev-police"]:setBeatmodeDebuff(true)
            end
...
AddEventHandler('doTimer', function()
    ...
    deathText = isMinor and ("Unconscious: ~r~ %d~w~ seconds remaining") or ("Dead: ~r~ %d ~w~ seconds remaining")
    respawnText = isMinor and ("~W~HOLD ~r~E ~w~ (%d) ~w~ TO ~r~GET UP~w~") or ("~w~HOLD ~r~E ~w~(%d) ~w~TO ~r~RESPAWN ~w~OR WAIT FOR ~r~EMS")
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["ev-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
function releaseBody(dropItems)
    ...
    TriggerServerEvent('ev-healthcare:playerRespawn', hosp)

    DoScreenFadeIn(3000)

    local wasBeatdown = exports["ev-police"]:getBeatmodeDebuff()
...
RegisterNetEvent('ev-death:alert')
AddEventHandler('ev-death:alert', function()
