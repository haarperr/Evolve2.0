...
function loadPublicData()
    local showWatermark = exports['ev-config']:GetMiscConfig('hud.watermark')
    ...
    local serverCode = exports["ev-config"]:GetServerCode()
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        SendUIMessage({ source = "ev-hud", app = "character", data = character });
        
        -- local endpointData = exports["ev-fiber"]:GetGameDetails()
        -- SendUIMessage({ source = "ev-hud", app = "game", data = endpointData });
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        SendUIMessage({ source = "fa-nui", app = "character", data = character });
        
        -- local endpointData = exports["fa-fiber"]:GetGameDetails()
        -- SendUIMessage({ source = "fa-nui", app = "game", data = endpointData });

        Citizen.Wait(5000)

        TriggerEvent('fa-fun:phoneReady')
        --TriggerServerEvent('fa-fun:phoneReady')
    end)
end
...
-- AddEventHandler("ev-ui:hud:values", function(values)
--   sendAppEvent("hud", {
--       food = values["hunger"],
