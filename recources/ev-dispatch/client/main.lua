...
AddEventHandler("ev-dispatch:openFull", function()
-- function openMenu()
            ...
            if IsPedShooting(ped) then
                ...
                if annen == 1 then
                    ...
                    if weaponSlec ~= `WEAPON_SNOWBALL` then
                        local job = exports["isPed"]:isPed("myjob")
                        if job ~= "taxi" and job ~= "sheriff" and job ~= "police2" then
                            if weaponSlec == `weapon_stungun` then
                                dispatch("10-10", "Stinger S-200(Taser)", false)
                            else
                                dispatch("10-11", "Gunshot Alert", false)
                            end
                            Citizen.Wait(5000)
                        end
                    end
...
AddEventHandler("dispatch:trigger-dispatch-client:custom", function(code,code1, name, street, alertcoordx, alertcoordy, time, icon, sa)
    ...
    if job == "police" then 
        if sa then 
            --print("police_vers")
            TriggerEvent('InteractSound_CL:PlayOnOne', "panicbutton", 0.5)
            SendNUIMessage({
                action = "add-dispatch",
                mod = "police",
                alertname = name,
