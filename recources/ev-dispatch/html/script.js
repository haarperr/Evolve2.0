...
function setCoords(x, y) {
    $.post('https://ev-dispatch/sev-coords', JSON.stringify({
        x: x,
        y: y
    }));
}
...
$(document).ready(function() {
	window.addEventListener('message', function(event){
		if (event.data.action == "open") {
            ...
            <li class="MuiButtonBase-root MuiListItem-root MuiMenuItem-root MuiMenuItem-gutters MuiListItem-gutters MuiListItem-button sev-gps" tabindex="-1" role="option" aria-disabled="false" data-value="10">
            Set GPS Location<span class="MuiTouchRipple-root">
            </span></li>`)
            $('.sev-gps').click(function() {
                $.post('https://ev-dispatch/sev-coords', JSON.stringify({
                    number: roninNumber
                }));
                $('.called-list').fadeOut();
            ...
            $('.sev-gps-location').click(function() {
                $.post('https://ev-dispatch/sev-coords', JSON.stringify({
                    number: roninNumber
                }));
        ...
        if (event.data.action == "close") {
            $.post('https://ev-dispatch/close', JSON.stringify({}));
            outKapa();
		}

        if (event.data.action == "close-callsign") {

            $.post('https://ev-dispatch/close-callsign', JSON.stringify({}));
            outKapa();
		}
        ...
        $('.create-call').click(function() {
            $.post('https://ev-dispatch/create_call', JSON.stringify({
                number: roninNumber
            }));
        })
        ...
        $('.dismiss-pingg').click(function() {
            createCall3.addClass('dismissed');
            $('.called-list').fadeOut();
            $.post('https://ev-dispatch/dismiss_call-2', JSON.stringify({
                number: roninNumber
            }));
        })
    ...
    function vehiclesClick() {
        $('.vehicles').click(function() {
            ...
            if (value == 1) {
                ...
                removeItem()
                mesaiHold()
                $.post('https://ev-dispatch/change-vehicle', JSON.stringify({
                    name: policeName,
                    icon: icon
                }));
                ...
                $.post('https://ev-dispatch/table_ayril', JSON.stringify({
                    name: remove
                }));
    ...
    $('.dismiss-ping').click(function() {
        ...
        $.post('https://ev-dispatch/dismiss_call-1', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    function assignButtons(){
        $('.assing-buttons').click(function() {
            $.post('https://ev-dispatch/add_police-assign', JSON.stringify({
    ...
    $('.crate-dismiss').click(function() {
        ...
        $.post('https://ev-dispatch/create_call-2', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    $('.remove-dismis-ping').click(function() {
        $('.dismiss-list').fadeOut();
        createCall4.remove();
        $.post('https://ev-dispatch/remove-call', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    function cureungotunu() {
        ...
        $('.remove-unit').click(function() {
            ...
            $.post('https://ev-dispatch/delete', JSON.stringify({
                name: player
            }));
            ...
            $('.mesai-personel').each(function() {
                if ($.trim($(this).text()) == text) {
                    ...
                    if (owner == player) {
                        ...
                        $.post('https://ev-dispatch/under-police', JSON.stringify({
                            player: player,
                            owner: owner
                        }));
                    }
    ...
    function callCameRemove(){
        $('.call-came').dblclick(function(e){
            ...
            $('.remove-list .MuiPaper-rounded ul li').click(function(){
                $.post('https://ev-dispatch/delete_police-assign', JSON.stringify({
                    ronin: callCame
                }));
            })
        })
    ...
    function save(){
        ...
        $.post('https://ev-dispatch/save', JSON.stringify({
            html: save
        }));
