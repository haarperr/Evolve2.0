...
AddEventHandler('chat:showCID', function(cidInformation, pid)
  ...
  local targetCoords = GetEntityCoords(targetPed)
    if pid ~= -1 then
        ...
        local shit = json.encode(cidInformation)
        local strng = "\n\n CID : " .. json.decode(shit).Identifier .. " \n Firstname : " .. json.decode(shit).Name .. " \n Lastname : " .. json.decode(shit).Surname .. " \n Date Of Birth : " .. json.decode(shit).DOB .. " \n Gender : " .. json.decode(shit).Sex .." \n\n"
        TriggerEvent("chatMessage", "CITIZEN ID", 4, strng)
      end
    end
end)
...
RegisterCommand('me', function(source, args)
    ...
    TriggerServerEvent('chat:me_log', text)
end)
