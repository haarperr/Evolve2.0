...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("mechanic:attemptPurchase",cheap, type, upgradeLevel)
...
AddEventHandler('event:control:bennys', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        elseif useID == 3 and not isPlyInBennys and exports["isPed"]:GroupRank("tuner_carshop") >= 3 then
            bennyHeading = 273.27651977539
...
function enterLocation(locationsPos)
    ...
    SetTimeout(100, function()
        if GetVehicleBodyHealth(plyVeh) < 2000.0 then
    ...
    while true do 
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            if nearDefault or nearImport or nearTuner or nearBridge or nearPaleto then
                ...
                if nearTuner and rankT >= 3 then
                    if not isPlyInBennys then
                      --  Draw3DText(bennyLocationTuner.x, bennyLocationTuner.y, bennyLocationTuner.z + 0.1, "[Press ~p~E~w~ - Enter Tuner Shop]", 255, 255, 255, 255, 4, 0.30, true, true, true, true, 0, 0, 0, 0, 55)
                    else
                       disableControls()
                    end
                end
...
AddEventHandler('ev-polyzone:enter', function(name)
    if name == "redline_bennys" then
        zone = true
        exports["ev-interface"]:showInteraction("[E] Redline Bennys")
    end
end)
