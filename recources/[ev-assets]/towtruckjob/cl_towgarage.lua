...
-- 		local finished = exports["ev-taskbar"]:taskBar(20000,"Changing plates", true)
...
AddEventHandler('towgarage:craft', function(res,amount)
	...
	if rank >= 4 or rank2 > 1 then -- Eddie wants rank 4 for crafting
		...
		local finished = exports["ev-taskbar"]:taskBar(1000,"Completing Task",false,true)

		if finished ~= 100 then
			return
		end
		if res == 1 and rank > 1 then
			local hasitems = exports["ev-inventory"]:hasEnoughOfItem("electronics",100*amount,true)
			if hasitems then
		...
		elseif res == 2 then
			...
			local hasitems = exports["ev-inventory"]:hasEnoughOfItem("copper",3*amount,true)
			if hasitems then
		...
		elseif res == 3 then
			...
			local hasitems = exports["ev-inventory"]:hasEnoughOfItem("aluminium",2*amount,true)
			if hasitems then
		...
		elseif res == 4 then
			...
			local hasitems = exports["ev-inventory"]:hasEnoughOfItem("steel",2*amount,true)
			if hasitems then
				TriggerEvent("inventory:removeItem","steel", 2*amount)
				TriggerEvent("player:receiveItem","lockpick",amount)
			end
		elseif res == 5 then
			local hasitems = exports["ev-inventory"]:hasEnoughOfItem("electronics",3*amount,true)
			if hasitems then
		...
		elseif res == 6 and rank > 1 then
			local hasitems = exports["ev-inventory"]:hasEnoughOfItem("rubber",10*amount,true)
			if hasitems then
...
AddEventHandler('towgarage:triggermenu', function(degradation,eHealth,bHealth)
	...
	if targetVehicle ~= nil  and targetVehicle ~= 0 then
		...
		local Internals = {
			{
				...
				description = "Plate: "..GetVehicleNumberPlateText(targetVehicle),
				children = {
					{
						title = "Brakes",
						description = "Current State: " .. round(degHealth["breaks"] / 10,2) .. "/10.0%",
						key = "brakes",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Axle",
						description = "Current State: " .. round(degHealth["axle"] / 10,2) .. "/10.0%",
						key = "axle",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Radiator",
						description = "Current State: " .. round(degHealth["radiator"] / 10,2) .. "/10.0%",
						key = "radiator",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Clutch",
						description = "Current State: " .. round(degHealth["clutch"] / 10,2) .. "/10.0%",
						key = "clutch",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Transmission",
						description = "Current State: " .. round(degHealth["transmission"] / 10,2) .. "/10.0%",
						key = "transmission",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Electronics",
						description = "Current State: " .. round(degHealth["electronics"] / 10,2) .. "/10.0%",
						key = "electronics",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Injector",
						description = "Current State: " .. round(degHealth["fuel_injector"] / 10,2) .. "/10.0%",
						key = "injector",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Fuel",
						description = "Current State: " .. round(degHealth["fuel_tank"] / 10,2) .. "/10.0%",
						key = "fuel",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Body",
						description = "Current State: " .. round((bodyHealth / 10) / 10,2) .. "/10.0%",
						key = "body",
						action = "ev-vehicles:fixPart"
					},
					{
						title = "Engine",
						description = "Current State: " .. round((engineHealth / 10) / 10,2) .. "/10.0%",
						key = "engine",
						action = "ev-vehicles:fixPart"
					},
				}
		...
		exports["ev-interface"]:showContextMenu(Internals)
	end
end)

RegisterInterfaceCallback('ev-vehicles:fixPart', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('towgarage:repairamount', data.key)
end)
...
AddEventHandler('towgarage:TriggerRepairs', function(degname,amount)
	...
	if not notfucked then
		...
		local finished = exports["ev-taskbar"]:taskBar(15000,"Repairing")
