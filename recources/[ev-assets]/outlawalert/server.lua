...
RegisterServerEvent('judgeAnnounce')
AddEventHandler('judgeAnnounce', function(msg)
	local msg = msg
    local searcher = source
    announcements = announcements + 1
    if announcements < 10 then
        TriggerClientEvent('chatMessage', -1, "^1[State Announcement]",{255, 0, 0}, msg)
    else
        TriggerClientEvent('chatMessage', -1, "^1No",{255, 0, 0},"Too many announcements sent.")
    end
end)

RegisterCommand('judgeannounce', function(source, args)
	local src = source
	local msg = ""
    for i = 1, #args do
      msg = msg .. " " .. args[i]
    end
	TriggerEvent('judgeAnnounce', msg)
end)
