...
local InjuryList = {
    ...
    [`WEAPON_SNSPISTOL_MK2`] = { "WEAPON_SNSPISTOL_MK2", "PaintBall Gun", minor = true },
...
function releaseBody(dropItems)
    ...
    if dropItems then
        ...
        --TriggerServerEvent("ev-ragdoll:removeallitems")
    end
...
AddEventHandler('ragdoll:sendPing', function()
    if not SentPing then
        SentPing = true
        exports["ev-dispatch"]:policedead('10-47', "Injured Civillian", "429")
        Citizen.Wait(10000)
        SentPing = false
    else
        TriggerEvent('DoLongHudText', 'You have already sent a ping wait a little bit', 2)
    end
end)
