...
function isPed(checkType)
    ...
    if checkType == "alias" then
        pass = RPC.execute("GetAlias")
    end
...
RegisterNetEvent('client:updatepaychk')
AddEventHandler('client:updatepaychk', function(lol)
  print('paycheck updated: ' .. lol)
  paycheck = lol
end)
...
RegisterNetEvent("updateAlias")
AddEventHandler("updateAlias", function(cidpassed)
    alias = cidpassed
end)
...
AddEventHandler("ev-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
AddEventHandler('ev-base:addedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)

RegisterNetEvent('ev-base:removedMoney')
AddEventHandler('ev-base:removedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)
