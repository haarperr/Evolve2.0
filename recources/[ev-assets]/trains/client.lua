...
AddEventHandler("AskForTrainConfirmed", function()
	...
	if metroDist < countryDist then
		if metroDist < 25.0 then 
			local finished = exports["ev-taskbar"]:taskBar(60000,"Waiting for Train",false,true)	
			if finished == 100 then
				StartMetro(closestMetro)
				TriggerEvent('chatMessage', 'THOMAS:', 4, ' Choo Choo..')
			end
		else
			TriggerEvent('chatMessage', 'THOMAS:', 4, ' You are not near a train station..')
		end
	else
...
function StartMetro(coords)
	...
	TriggerEvent('chatMessage', 'THOMAS:', 4, ' Train will depart in 30 seconds.')
