...
local logs = "https://discord.com/api/webhooks/1012082555232723004/fhXkIuiJneTbG_xgnuFqqKcInTRaVgeUk2iRda1OB5FIvgKTL_9tL28jRJL357UA2VKX"

local kick_msg = "Not today, go get a life touch some grass and learn how to dev."
...
AddEventHandler(GetCurrentResourceName(), function()
	...
	for _, v in pairs(allowlist) do
     ...
     elseif checkmethod == 'SQL' then
        MySQL.Async.fetchAll("SELECT rank FROM users WHERE identifier = @identifier",{['@identifier'] = identifierDb }, function(results) 
            if results[1].group ~= 'dev' or 'owner' then
