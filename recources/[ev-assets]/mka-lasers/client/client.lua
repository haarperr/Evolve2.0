...
local function calculateCurrentPoint(fromPoint, toPoint, deltaTime, travelTimeBetweenTargets)
  local evdDirection = toPoint - fromPoint
  local evdDirectionDist = #evdDirection
  local percentOfTravelTime = deltaTime / (travelTimeBetweenTargets * 1000)
  local distance = math.min(evdDirectionDist * percentOfTravelTime, evdDirectionDist)
  return fromPoint + (norm(evdDirection) * distance)
end
