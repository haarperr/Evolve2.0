...
function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		SetFuel(vehicle, CurrentFuel(vehicle) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (Config.Classes[GetVehicleClass(vehicle)] or 1.0) / 10)
	end
end
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	currentFuel = CurrentFuel(vehicle)
			...
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 2.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if CurrentFuel(vehicle) < 95 and canFuel then
	...
	Citizen.CreateThread(function()
		while true do
			...
			if IsPedInAnyVehicle(ped) and not (Config.RemoveHUDForBlacklistedVehicle and inBlacklisted) then
				...
				fuel = tostring(math.ceil(CurrentFuel(vehicle)))
