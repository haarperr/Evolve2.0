...
function HealSlow(time, healamount)
  Citizen.CreateThread(function()
    ...
    while healtime > 0 do
        Citizen.Wait(1000)
        healtime = healtime - 1
        SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) + (healamount and healamount or 1))
    end
...
AddEventHandler("healed:useOxy", function(isOxy, ignoreAddiction)
    ...
    if isOxy and isOxy ~= -1 and (not ignoreAddiction) then
      local addictionFactor = exports["ev-drugeffects"]:getOwnAddictionFactor("oxy")
...
AddEventHandler("DoHealthFading",function()
    ...
    maxfade = math.ceil(200.0 - (health)) + 50
	if maxfade > 240 then
		maxfade = 240
	end
