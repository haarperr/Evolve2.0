...
RegisterNetEvent("AnimSet:sk1c2");
AddEventHandler("AnimSet:sk1c2", function()
...
RegisterCommand('e', function(source, args, raw) 
    ...
    if not IsPedSittingInAnyVehicle(PlayerPedId()) and not dead and not inwater then
    TriggerEvent('animation:PlayAnimation', args[1]) 
    else 
        TriggerEvent("DoLongHudText", "Can't Emote At This Time",2)
    end
end)
