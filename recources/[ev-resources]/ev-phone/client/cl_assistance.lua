...
AddEventHandler('phone:assistClear', function(id,jobsent)
  TriggerEvent('jobssystem:current', function(job)
    if myBlips[id] ~= nil and not myBlips[id].onRoute and (job == jobsent or (job == "police" or job == "sheriff" or job == "state" and jobsent == "911")) then
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if nearEntertainment() and exports["ev-base"]:getModule("LocalPlayer"):getVar("job") == "entertainer" then
    ...
    elseif exports["ev-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
      local dist = 0
      if lastBlip.x then
      ...
      if exports["ev-base"]:getModule("LocalPlayer"):getVar("job") == "entertainer" then
        Citizen.Wait(300000)
      end
    end
