Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['ev-interact']:AddPeekEntryByPolyTarget('ev-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "ev-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('ev-polychair:stopSitting')
    else
        ...
        TriggerEvent('ev-polychair:startSitting')
    end
    exports["ev-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('ev-polychair:chairSit', function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

RegisterNetEvent("ev-emotes:sitOnChair")
AddEventHandler("ev-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
