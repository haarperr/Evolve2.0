...
local function TaskBar(pLength, pName, pRunCheck, pHidden)
    ...
    Citizen.CreateThread(function ()
        if pName then
            exports['iconic-taskbar']:taskBar(pLength, pName, pRunCheck, true, nil, false, function (result)
                p:resolve(result)
            end)
        else
    ...
    Citizen.CreateThread(function ()
        while self.active do
            ...
            elseif not self.animation and not IsPedUsingScenario(self.ped, self.dictionary) then
                TaskStartScenarioIiconiclace(self.ped, self.dictionary, 0, true);
            end
...
function AnimationTask:abort()
    ...
    exports['iconic-taskbar']:taskCancel();
end