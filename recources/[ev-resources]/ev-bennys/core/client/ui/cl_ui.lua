...
function InitiateMenus(isMotorcycle, vehicleHealth)
    ...
    if vehicleHealth < 1000.0 then
        ...
        TriggerServerEvent("ev-bennys:updateRepairCost", repairCost)
    ...
    if not isMotorcycle then
        if vehclass == 18 or devmode or allowed or hayes then
            ...
            elseif isMenuActive("NeonsSideMenu") then
                ...
                if currentMenu == "repairMenu" then
                    if AttemptPurchase("repair") then
                        ...
                        if vehclass == 18 and bennysAccess == 'police' then
                            TriggerServerEvent("ev-bennys:resetDegredation", VehToNet(plyVeh))
...
RegisterNUICallback("selectedItem", function(data, cb)
    print(tonumber(data.id), data.item, data.item2)
