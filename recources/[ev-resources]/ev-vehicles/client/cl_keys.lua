...
AddEventHandler("ev-jobmanager:playerBecameJob", function(job)
  ...
  if job == "towtruck" or job == "police" or job == "ems" or job == "sheriff" or job == "state" then 
    towavail = true 
  else
    towavail = false 
  end
          ...
          if not hasKey(plate1) then
            ...
            if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
              ...
              if IsEntityDead(pedDriver) then
                ...
                TriggerEvent('ev-dispatch:carjacking')
                local finished = exports["ev-taskbar"]:taskBar(3000,"Taking Keys",false)
                ...
                if GetEntityModel(curveh) ~= `taxi` then
                  ...
                  if math.random(100) > 95 then
                      ...
                      TriggerEvent('ev-dispatch:carjacking')
                      local finished = exports["ev-taskbar"]:taskBar(3000,"Taking Keys")
                      if finished == 100 then
                       -- SetEntityAsMissionEntity(curveh,false,true)
                        TriggerEvent("keys:addNew",curveh,plate1)
                      else
