...
function DPX.Jobs.CanBecomeJob(self, user, job, callback)
    ...
    if not hexId or not characterId or not src then callback(false, "Id's don't exist") return end
        ...
        TriggerEvent("ev-jobmanager:attemptBecomeJob", src, characterId, function(allowed, reason)
...
function DPX.Jobs.SetJob(self, user, job, notify, callback)
    ...
    DPX.Jobs:CanBecomeJob(user, job, function(allowed, reason, rank) 
        ...
        TriggerClientEvent("ev-jobmanager:playerBecameJob", src, job, name, false)
        TriggerClientEvent("ev-jobmanager:playerBecomeEvent", src, job, name, notify)
...
AddEventHandler("ev-base:exportsReady", function()
    exports["ev-base"]:addModule("JobManager", DPX.Jobs)
end)
...
RPC.register("ev-jobmanager:jobCount", function(pSource, pJob)
    return DPX.Jobs:CountJob(pJob)
end)
