...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags['isJobEmployer'] then
        pContext.job = exports['ev-jobs']:GetNPCJobData(npcId)
    end
end
...
function GetEntityContext(pEntity)
    ...
    if pEntity then
        ...
        if context.type == 1 then
            context.flags = exports['ev-flags']:GetPedFlags(pEntity)
        ...
        elseif context.type == 2 then
            context.flags = exports['ev-flags']:GetVehicleFlags(pEntity)
        elseif context.type == 3 then
            context.flags = exports['ev-flags']:GetObjectFlags(pEntity)
        end
...
--     exports["ev-binds"]:registerKeyMapping("Menu", "Show Menu", "+showFastMenu", "-showFastMenu", "F1")
-- end)
...
RegisterCommand("fixfocus", function()
    SetNuiFocus(false, false)
    TriggerEvent("DoLongHudText", "F1 Menu Reset")
end, false)
