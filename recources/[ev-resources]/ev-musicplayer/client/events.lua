RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("ev-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("ev-musicplayer:stateSound")
AddEventHandler("ev-musicplayer:stateSound", function(state, data)
