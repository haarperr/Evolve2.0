...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('ev-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('ev-hospital:client:SyncBleed')
AddEventHandler('ev-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('ev-hospital:client:FieldTreatLimbs')
AddEventHandler('ev-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('ev-hospital:client:FieldTreatBleed')
AddEventHandler('ev-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('ev-hospital:client:ReduceBleed')
AddEventHandler('ev-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('ev-hospital:client:ResetLimbs')
AddEventHandler('ev-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('ev-hospital:client:RemoveBleed')
AddEventHandler('ev-hospital:client:RemoveBleed', function()
    isBleeding = 0
