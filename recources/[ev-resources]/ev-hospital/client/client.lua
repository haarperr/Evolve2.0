...
AddEventHandler("ev-hospitalization:checkin", function(allowed)
	if allowed then
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["ev-taskbar"]:taskBar(2500,"Checking Credentials")
...
RegisterNetEvent('attempt:check-in', function()
	TriggerServerEvent('attempt:check-in')
end)
...
AddEventHandler("ev-hospitalization:page", function()
	if exports["isPed"]:isPed("countems") >= 1 then 
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["ev-taskbar"]:taskBar(2500,"Paging a Doctor")
		if finished == 100 then
...
Citizen.CreateThread(function()
exports["ev-polytarget"]:AddCircleZone("pillbox_checkin", vector3(307.59, -595.3, 43.08), 0.35, {
    name="pillbox_checkin",
  })
end)
...
Citizen.CreateThread(function()
    exports["ev-polyzone"]:AddBoxZone("pillbox_ems_equipment", vector3(306.72, -602.02, 43.28), 1, 2.4, {
...
EMSEquipmentZone = false

RegisterNetEvent('ev-polyzone:enter')
AddEventHandler('ev-polyzone:enter', function(name)
    if name == "pillbox_ems_equipment" then
        EMSEquipmentZone = true     
        EMSEquipment()
        if exports['isPed']:isPed('myjob') == "ems" then
            exports['ev-interface']:showInteraction("[E] Equipment")
        end
    end
end)
...
function EMSEquipment()
	Citizen.CreateThread(function()
        while EMSEquipmentZone do
            Citizen.Wait(5)
