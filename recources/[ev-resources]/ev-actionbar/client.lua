...
RegisterNetEvent('ev-items:SetAmmo')
AddEventHandler('ev-items:SetAmmo', function(sentammoTable)
...
function actionBarDown()
  ...
  TriggerServerEvent("ev-financials:cash:get", GetPlayerServerId(PlayerId()))
end
...
Citizen.CreateThread(function()
	exports["ev-keybinds"]:registerKeyMapping("", "Player", "Action Bar", "+actionBar", "-actionBar", "TAB")
	RegisterCommand('+actionBar', actionBarDown, false)
	RegisterCommand('-actionBar', actionBarUp, false)

	exports["ev-keybinds"]:registerKeyMapping("", "Player", "Wink", "+playerWink", "-playerWink")
	RegisterCommand('+playerWink', playerWink, false)
	RegisterCommand('-playerWink', function() end, false)

	exports["ev-keybinds"]:registerKeyMapping("", "Player", "Passive Mode", "+passiveMode", "-passiveMode")
...
AddEventHandler('ev-actionbar:hotreload', function()
...
Citizen.CreateThread( function()
	...
	while true do
      ...
      if weapon == "-37975472" then
        TriggerEvent("ev-weapons:threwSmokeGrenade")
        if lastEquippedItemToRemove then
          if exports["ev-inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["ev-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["ev-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("ev-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID,itemToRemove)
	--GiveAmmoNow()
	if not exports["ev-propattach"]:canPullWeaponHoldingEntity() then return end
  ...
  lastEquippedItemToRemove = itemToRemove
	...
	TriggerEvent("ev-weapons:client:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
	TriggerServerEvent("ev-weapons:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("ev-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('ev-item:CheckClientAmmo')
AddEventHandler('ev-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["ev-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["ev-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["ev-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["ev-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["ev-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["ev-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["ev-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
...
RegisterNetEvent("ev-weapons:hitPlayerWithCash", function(pTarget)
  TriggerServerEvent("ev-weapons:processGiveCashAmount", pTarget, lastEquippedInfo.amount, lastEquippedInfo.id)
end)
