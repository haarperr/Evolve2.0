...
on('ev-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://ev-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['ev-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
  ...
  //  const apartmentFloor = exports["ev-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['ev-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
       ...
       // let serverCode = exports["ev-config"].GetServerCode();
...
on('ev-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
    ...
    if (MyInventory) {
        ...
        try {
            return (JSON.parse(CacheBinds(JSON.parse(MyInventory))) && !!CacheBinds(JSON.parse(MyInventory)));
        } catch (e) {
            return false;
        };
    }
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('ev-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("ev-inventory:craftProgression", data);
    emitNet("ev-inventory:craftProgression", data);
});
...
on('__cfx_nui:move', (data, cb) => {
    if (global.exports['ev-taskbar'].pInInv()) {
        emit('DoLongHudText', 'Nice try exploiting mate.', 2)
        emitNet('ev-inventory:exploiterKick')
    } else {
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("ev-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("ev-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('ev-inventory:server:weightChange', cid, weight);
