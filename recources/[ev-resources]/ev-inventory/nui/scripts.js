...
$(document).ready(function() {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://ev-inventory/UpdateSettings',
            JSON.stringify({
...
function invStack(
    ...
    $.post('https://ev-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://ev-inventory/move', JSON.stringify(arr));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0, targetInvSlots = 40) {
    ...
    if (targetinvName.indexOf('Drop') > -1) {
    ...
    } else if (targetinvName.startsWith('saints-stash')) {
        secondaryMaxWeight = 5000.0;
        slotLimitTarget = 100;
        displayName = 'Saints';
    ...
    } else if (targetinvName.startsWith('maldinis-')) {
        secondaryMaxWeight = 50.0;
        slotLimitTarget = 5;
        displayName = 'Maldinis Table';
        ...
        secondaryMaxWeight = 5000.0;
        slotLimitTarget = 100;
        displayName = 'Mechanic Stash';
    } else if (targetinvName.indexOf('harmony-stash') > -1) {
        secondaryMaxWeight = 5000.0;
        slotLimitTarget = 100;
        displayName = 'Mechanic Stash';
    } else if (targetinvName.indexOf('hayes-stash') > -1) {
        secondaryMaxWeight = 5000.0;
        slotLimitTarget = 100;
        displayName = 'Mechanic Stash';
    } else if (targetinvName.indexOf('stash-winery') > -1) {
        secondaryMaxWeight = 5000.0;
        slotLimitTarget = 100;
        displayName = 'Johnathan Rose Loves Cock';
    } else if (targetinvName.indexOf('stash-white-widow') > -1) {
        secondaryMaxWeight = 5000.0;
        slotLimitTarget = 100;
        displayName = 'Johnathan Rose Loves Cock';
    } else if (targetinvName.indexOf('ottos-autos-stash') > -1) {
        secondaryMaxWeight = 5000.0;
        slotLimitTarget = 100;
        displayName = 'Mechanic Stash';
    } else {
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://ev-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://ev-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://ev-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://ev-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://ev-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://ev-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://ev-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://ev-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://ev-inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://ev-inventory/craftProgression', JSON.stringify({
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://ev-inventory/invuse', JSON.stringify(arr));
