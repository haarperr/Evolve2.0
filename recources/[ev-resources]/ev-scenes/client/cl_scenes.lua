...
function calculateScenesToDraw()
  ...
  for _, scene in pairs(activeScenes) do
    local coord = json.decode(scene.coords)
    local acCoord = vector3(coord.x, coord.y, coord.z)
    if ((tonumber(scene.solid) == 1 and isPeekActive) or tonumber(scene.solid) == 0) and #(acCoord - playerCoords) < tonumber(scene.distance) then
      if not scene.processed then
        ...
        SetTextFont(tonumber(scene.font) and tonumber(scene.font) or 4)
        SetTextCentre(true)
        local swidth = 0
  ...
  while true do
    ...
    if scenesEnabled then
      ...
      for _, scene in pairs(drawnScenes) do
        ...
        DrawText3D(coord.x, coord.y, coord.z, dist, scene.pText, scene.color, scene.background, scene.font, tonumber(scene.caret), opacity)
      end
...
RegisterInterfaceCallback("ev-ui:scenes:input", function(data, cb)
...
RegisterCommand("+deleteScene", function()
  -- loops thru all scenes, checks closeset deletes it from db and force re-calc
