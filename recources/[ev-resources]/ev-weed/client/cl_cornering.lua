...
local InteractStartCorner = {
    ...
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["ev-inventory"]:hasEnoughOfItem("weedq", 1, false, true)
...
AddEventHandler("ev-weed:startCornering", function(pContext, pEntity)
    ...
    local canCorner, message, engMessage = RPC.execute("ev-weed:startCorner", CurrentCornerCoords)
...
AddEventHandler('ev-weed:cornerSale', function(pContext, pEntity)
    ...
    local hasBaggies = exports['ev-inventory']:hasEnoughOfItem('weedq', 1, true)
    ...
    if didSale then
      ...
      if rndAmt == 1 and exports['ev-inventory']:hasEnoughOfItem('weedq', 1) then
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 2 and exports['ev-inventory']:hasEnoughOfItem('weedq', 3) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 3 and exports['ev-inventory']:hasEnoughOfItem('weedq', 2) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
...
AddEventHandler("ev-weed:prepareBaggies", function(pContext, pEntity)
    ...
    local finished = exports["ev-taskbar"]:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
    if finished == 100 then
        ...
        RPC.execute("ev-weed:prepareBaggies", info)
    end
end)
