...
RegisterNetEvent('ev-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('ev-voice:setTalkingOnRadio', setTalkingOnRadio)
...
function addPlayerToRadio(plySource)
	TriggerEvent('ev-voice:getRadioChannel')
...
RegisterNetEvent('ev-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('ev-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('ev-voice:setPlayerRadio', channel)
	...
	if not exports['ragdoll']:GetDeathStatus() then 
		if not radioPressed and radioEnabled then
			if radioChannel > 0 then
				...
				playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
				TriggerServerEvent('ev-voice:setTalkingOnRadio', true)
				radioPressed = true
				playMicClicks(true)
				...
				Citizen.CreateThread(function()
					TriggerEvent('ev-hud:transmitting', radioPressed)
					TriggerEvent("ev-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		playerTargets(NetworkIsPlayerTalking(PlayerId()) and callData or {})
		TriggerEvent("ev-voice:radioActive", false)
		TriggerEvent('ev-hud:transmitting', radioPressed)
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 1) == 1 then
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
		end
		TriggerServerEvent('ev-voice:setTalkingOnRadio', false)
	end
end, false)
...
RegisterNetEvent('ev-voice:clSetPlayerRadio', syncRadio)

RegisterNetEvent("ev-voice:getRadioChannel")
AddEventHandler("ev-voice:getRadioChannel", function()
	TriggerServerEvent('ev-dispatch:GrabRadioChannel', radioChannel)
end)