if not IsDuplicityVersion() then
    ...
    function SetCustomNuiFocus(hasKeyboard, hasMouse)
        ...
        TriggerEvent("ev-base:focus", HasNuiFocus, hasKeyboard, hasMouse)
    end
    ...
    function TaskItem(item, animDict, animName, typeAnim, speed, prop, message, timer, movement, weapon, remove)
        if not LocalPlayer.state.doge then
            TriggerEvent('ev-lib:forceAnimation', animDict, animName, typeAnim, speed, prop)
        end
        local finished = exports["ev-taskbar"]:TaskBar(message, timer, movement, weapon)
        if finished == 100 then
            if exports["ev-inventory"]:HasItem(item) then
                if remove then
                    TriggerEvent("inventory:destroy", item)
                end
                TriggerEvent('ev-lib:forceAnimation')
                return true
            else
    ...
    function GetNearbyPeds(X, Y, Z, Radius)
        ...
        if tonumber(X) and tonumber(Y) and tonumber(Z) then
            if tonumber(Radius) then
                for Ped in exports["ev-admin"]:EnumeratePeds() do
    ...
    function SkillCircleLooped(count)
        ...
        while loop < count do
            loop = loop + 1
            local finished = exports["ev-skillcircle"]:SkillCircle(math.random(13, 60))
    ...
    function SpeedSkillCircle()
        local finished = exports["ev-skillcircle"]:SkillCircle(math.random(4, 7))
        if not finished then
            return false
        end
        return true
    ...
    function SkillCircleLooped2(sent)
        ...
        for i = 1, #speeds do
            local finished = exports["ev-skillcircle"]:SkillCircle(speeds[i])
            if not finished then
                return false
            end
            Wait(200)
    ...
    function SpawnVehicle(model, spawn, heading)
        ...
        TriggerEvent('ev-vmod:addKeys', _vehicle)
