...
function joinRace(src, id, alias, cid, dontpay)
    ...
    TriggerClientEvent("ev-racing:joinedRace", src, Races[id])
    TriggerClientEvent("ev-racing:addedPendingRace", -1, PendingRaces[id])

    for k, v in pairs(PendingRaces[id]["players"]) do
        TriggerClientEvent("ev-racing:playerJoinedYourRace", v.serverId, cid, alias)
    end
end
...
RegisterNetEvent("ev-racing:updatePositionInfo")
AddEventHandler("ev-racing:updatePositionInfo", function(curRaceid, totalDist)
    local src = source
    local srcString = tostring(src)
    ...
    TriggerClientEvent("ev-racing:updatePosition", src, position)
end)
...
RPC.register("ev-racing:createPendingRace", function(src, ids, optionss)
    ...
    TriggerClientEvent("ev-racing:addedPendingRace", -1, options)
    TriggerClientEvent('ev-racing:pendingRacenotif', -1)
    print("GETTING TO JOIN AFTER CREATING RACE",options)
    joinRace(src, id, alias, cid, true)
end)

RPC.register("ev-racing:joinRace", function(src, ids, aliass, cids)
    local id = ids
    local alias = aliass
    local cid = cids
...
RPC.register("ev-racing:leaveRace", function(src)
    local srcString = tostring(src)
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if cid == PendingRaces[raceId]["owner"] then
        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("ev-racing:endRace", v.serverId)
        end

        PendingRaces[raceId] = nil
        TriggerClientEvent("ev-racing:removedPendingRace", -1, raceId)
    else
        PendingRaces[raceId]["players"][srcString] = nil
        TriggerClientEvent("ev-racing:leftRace", src)
        TriggerClientEvent("ev-racing:addedPendingRace", -1, PendingRaces[raceId])

        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("ev-racing:playerLeftYourRace", v.serverId, cid, alias)
        end
    end
end)

RPC.register("ev-racing:dnfRace", function(src, ids)
    local id = ids
    local srcString = tostring(src)
    ...
    TriggerClientEvent("ev-racing:leftRace", src)
    TriggerClientEvent("ev-racing:updatedActiveRace", -1, ActiveRaces[id])
    ...
    if ActiveRaces[id]["positions"] > countPlayers then
        ...
        TriggerClientEvent("ev-racing:raceHistory", -1, ActiveRaces[id])
        ...
        ActiveRaces[id] = nil
        TriggerClientEvent("ev-racing:removedActiveRace", -1, id)
    end
end)

RPC.register("ev-racing:startRace", function(src, ids, countdowns, ownerID)
    ...
    TriggerClientEvent("ev-racing:addedActiveRace", -1, ActiveRaces[id])

    PendingRaces[id] = nil
    TriggerClientEvent("ev-racing:removedPendingRace", -1, id)
    ...
    for k, v in pairs(ActiveRaces[id]["players"]) do
        ...
        if isStartEnabled then
            TriggerClientEvent("ev-racing:startRace", v.serverId, race, countdown * 1000)
            TriggerClientEvent("ev-racing:api:currentRace", v.serverId, race)
        else
            local user = exports['ev-base']:getModule("Player"):GetUser(v.serverId)
            local char = user:getCurrentCharacter()
...
RPC.register("ev-racing:endRace", function(src)
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    ...
    TriggerClientEvent("ev-racing:removedActiveRace", -1, raceId)
end)

RPC.register("ev-racing:finishedRace", function(src, curRaceids, bestLapTimes, totalTimes)
    ...
    TriggerClientEvent("ev-racing:finishedRace", src, ActiveRaces[curRaceid]["players"][srcString]["position"], totalTime, ActiveRaces[curRaceid]["buyIn"])
    TriggerClientEvent("ev-racing:updatedActiveRace", -1, ActiveRaces[curRaceid])
    ...
    if ActiveRaces[curRaceid]["players"][srcString]["position"] == countPlayers then
        ...
        TriggerClientEvent("ev-racing:raceHistory", -1, ActiveRaces[curRaceid])
        ...
        TriggerClientEvent("ev-racing:removedActiveRace", -1, curRaceid)
    elseif ActiveRaces[curRaceid]["dnfPosition"] > 0 and ActiveRaces[curRaceid]["dnfPosition"] == ActiveRaces[curRaceid]["players"][srcString]["position"] then
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            TriggerClientEvent("ev-racing:startDNFCountdown", v.serverId, countdown)
        end
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            if v.position == 0 then
                TriggerClientEvent("ev-racing:dnfRace", v.serverId)
            end
        end
