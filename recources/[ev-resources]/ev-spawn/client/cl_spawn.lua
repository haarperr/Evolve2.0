...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["ev-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		events:Trigger("ev-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
	...
	if data.action == "newCharacter" then
		...
		TriggerServerEvent('ev-spawn:createCharacter:log', cData.first_name, cData.last_name, cData.dob, pGender)
		
        if not chardata then return end

		events:Trigger("ev-base:createCharacter", chardata, function(created)
            if not created then
...
RegisterNetEvent("ev-base:spawnInitialized")
