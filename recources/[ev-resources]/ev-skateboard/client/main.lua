...
RegisterNetEvent('ev-longboard:PlaceDown')
AddEventHandler('ev-longboard:PlaceDown', function()
	TriggerEvent('inventory:removeItem', 'skateboard', 1)
	TriggerEvent('DoLongHudText', '[E] To pick the skateboard back up.', 1)
	RCCar.Start()
end)
...
RCCar.HandleKeys = function(distanceCheck)
	...
	if distanceCheck < 100.0 then
		local overSpeed = (GetEntitySpeed(RCCar.Entity)*3.6) > 20
		...
		if IsControlPressed(0, 150) and not IsControlPressed(0, 149) and not overSpeed then
		...
		if IsControlPressed(0, 22) and Attached then
			...
			if IsControlJustReleased(0, 150) or IsControlJustReleased(0, 151) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 6, 2500)
			end

			if IsControlPressed(0, 151) and not IsControlPressed(0, 150) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 22, 1)
			end

			if IsControlPressed(0, 147) and IsControlPressed(0, 151) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 13, 1)
			end

			if IsControlPressed(0, 148) and IsControlPressed(0, 151) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 14, 1)
			end

			if IsControlPressed(0, 150) and IsControlPressed(0, 151) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 30, 100)
			end

			if IsControlPressed(0, 147) and IsControlPressed(0, 150) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 7, 1)
			end

			if IsControlPressed(0, 148) and IsControlPressed(0, 150) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 8, 1)
			end

			if IsControlPressed(0, 147) and not IsControlPressed(0, 150) and not IsControlPressed(0, 149) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 4, 1)
			end

			if IsControlPressed(0, 148) and not IsControlPressed(0, 150) and not IsControlPressed(0, 149) and not overSpeed then
