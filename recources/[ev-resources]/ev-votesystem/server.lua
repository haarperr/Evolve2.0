...
function gangCoords(gangType, src)
    ...
    exports.ghmattimysql.execute("SELECT coords FROM " .. gangtable .. "HERE id = @id", {
        ['id'] = 1
    }, function(result)
...
function CompleteUpdate(currentFunds)
    ...
    exports.ghmattimysql:execute("UPDATE general_variables SET value = @value WHERE id = @name", {
        ['value'] = currentFunds,
        ['name'] = 3
    })
end
...
AddEventHandler('updatecoordsweed', function(gangType, newCoords)
    exports.ghmattimysql:execute("UPDATE weed_farming SET coords = @newCoords WHERE id = @id", {
        ['newCoords'] = newCoords,
        ['id'] = 1
    })
...
function updateCoords(gangType, newCoords)
    ...
    exports.ghmattimysql.execute("UPDATE" .. gangtable .. " SET coords = @newCoords WHERE id = @id", {
        ['newCoords'] = newCoords,
        ['id'] = 1
    })
    TriggerClientEvent("gang:setcoords", -1, newCoords, gangType)
...
function updateMayorId()
    exports.ghmattimysql:execute("SELECT value FROM general_variables WHERE id = @id", {
        ['id'] = 2
    }, function(result)
...
AddEventHandler('group:pullinformation', function(groupid, rank)
    ...
    exports.ghmattimysql:execute(
        "SELECT * FROM character_passes WHERE pass_type = @groupid and rank > 0 ORDER BY rank DESC", {
            ...
            if results ~= nil then
                exports.ghmattimysql:execute("SELECT bank FROM group_banking WHERE group_type = @groupid", {
                    ['groupid'] = groupid
                }, function(result)
                    local bank = 0
                    if result[1] ~= nil then
...
AddEventHandler('server:givepayJob', function(pJob, pAmount, pSource)
    ...
    local char = user:getCurrentCharacter()
    local cid = tonumber(char.id)
    exports.ghmattimysql:execute("SELECT id, paycheck FROM characters WHERE id = @id", {
        ['id'] = cid
    }, function(result)
        if result[1] ~= nil then
    ...
    exports.ghmattimysql:execute("SELECT id,paycheck FROM characters WHERE id = @id", {
        ...
        if result[1] ~= nil then
            ...
            if curPaycheck > 0 then
                exports.ghmattimysql:execute("UPDATE characters SET paycheck = 0 WHERE id  = @id", {
                    ['id'] = cid
                })
...
AddEventHandler('server:givepayGroup', function(groupname, amount, cid)
    ...
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    exports.ghmattimysql:execute("SELECT bank FROM group_banking WHERE group_type = @id", {
        ['id'] = groupname
    }, function(data)
...
AddEventHandler('server:gankGroup', function(groupid, amount)
    ...
    if user:getCash() >= tonumber(amount) then
        ...
        exports.ghmattimysql:execute("SELECT bank FROM group_banking WHERE group_type = @id", {
            ...
            exports.ghmattimysql:execute("UPDATE group_banking SET `bank` = @bank WHERE `group_type` = @id", {
                ['id'] = groupid,
                ['bank'] = solution
            })
...
AddEventHandler('server:currentpassestgt', function(tgt)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_passes WHERE cid = @cid", {
        ['cid'] = char.id
    }, function(result)
        if result[1] ~= nil then
...
AddEventHandler('checkmayorname', function()
    ...
    exports.ghmattimysql:execute("SELECT first_name, last_name FROM characters WHERE id = @id", {
        ['id'] = mayorid
    }, function(result)
...
AddEventHandler('server:PayStoreOwner', function(cid, cashamount)
    ...
    exports.ghmattimysql:execute("SELECT id,paycheck FROM characters WHERE id = @cid", {
        ['cid'] = cid
    }, function(result)
        if result[1] ~= nil then
            local curPaycheck = result[1].paycheck
            ...
            exports.ghmattimysql:execute("UPDATE characters SET paycheck = @curPayCheck WHERE id = @cid", {
                ['cid'] = cid,
...
AddEventHandler('server:GroupPayment', function(gangid, cashamount, sauce)
    ...
    exports.ghmattimysql:execute("SELECT bank FROM group_banking WHERE group_type = @gangid", {
        ['gangid'] = gangid
    }, function(result)
        ...
        if result[1] ~= nil then
            ...
            exports.ghmattimysql:execute("UPDATE group_banking SET bank = @bank WHERE group_type = @gangid", {
                ['gangid'] = gangid,
                ['bank'] = curbank
            })
        end
