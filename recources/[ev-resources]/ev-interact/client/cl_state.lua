...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('ev-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("ev-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['ev-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("ev-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("ev-polyzone:enter", function(zone, data)
    ...
    if zone == "bennys" then
        ...
        if data and data.type == "redline" and not IsPedInAnyVehicle(plyPed) then
            return
        end
...
AddEventHandler("ev-fishing:currentlyRentedBoat", function(handle)
