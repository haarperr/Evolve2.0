...
AddEventHandler("ev-housing:sell", function(pCid, pPropertyId)
    ...
    print('here')
    TriggerClientEvent("ev-housing:buy", pCid, pPropertyId, seller)
end)
...
RPC.register("ev-housing:buy", function(src, pPropertyId, pTotal, pTax, pSeller)
    local user = exports['ev-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    ...
    TriggerClientEvent("ev-housing:loadHousingClient",src)
    TriggerClientEvent('ev-housing:logShit', src, pPropertyId, pTotal)
...
RPC.register("ev-housing:rent", function(src, pPropertyId, pTotal, pTax)
    ...
    --exports["ev-financials"]:addTax("Propertys", pTax)
...
RPC.register("ev-housing:isHousingLockedServer", function(pSrc, pHid)
    print(pHid)
...
RegisterNetEvent("ev-housing:getGarages", function()
    local src = source
    ...
    for i, v in ipairs(houses) do
        ...
        TriggerClientEvent("menu:send:ev-housing:garages", src,json.decode(garagedata),v.hid)
    end
end)