...
RegisterNetEvent("ev-base:sessionStarted", function()
    local src = source
    TriggerClientEvent("ev-housing:beingRobbed", src, Housing.housingBeingRobbed)
end)
...
RPC.register("housing:robbery:pickedLock", function(src, pHouseId)
    ...
    TriggerClientEvent("ev-housing:beingRobbed", -1, pHouseId)
...
RPC.register("property:alarmDisabled", function(src, pHouseId, pFail)
    ...
    for k, v in pairs(Housing.Players) do
        if v == pHouseId then
            ...
            if pFail then
                TriggerClientEvent("ev-housing:alarm", src, pHouseId)
            end
        end
    end
