...
GetUserByCid = function(cid)
    for index, value in pairs(GetPlayers()) do
        local xPlayer = exports['ev-base']:getModule("Player"):GetUser(tonumber(value))
...
RPC.register("ev-housing:isThisPropertyOwned", function(src, propertyId)
    local result = MySQL.query.await([[
...
RPC.register("ev-phone:giveKey", function(src, pHouseId, pPlayerId)
    ...
    local user2 = exports['ev-base']:getModule("Player"):GetUser(pPlayerId)
    ...
    TriggerClientEvent("ev-phone:notification", pPlayerId, "fas fa-key", "Keys", "Você recebeu a chave da propriedade " .. Housing.info[pHouseId]["street"], 5000)
...
RPC.register("ev-phone:payHouse", function(src, pHouseId)
  ...
  --  local tax = price
   ...
   -- local bank = exports["ev-financials"]:getBalance(accountId)
   ...
   --     return false, "Você não tem $" .. priceTotal .. " em sua conta"
    ...
    --exports["ev-financials"]:addTax("Propertys", tax.tax)
...
RegisterServerEvent('ev-housing:buyLog')
AddEventHandler('ev-housing:buyLog', function(pHouse, price)
    local src = source
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/1012082840038555690/uBrYz4sosUN9pwxpHI6h13f87WB8HQzvUcVFPhXTI6xJOsvsbltkRUGccok-4ICS51PG", function(err, text, headers) end, 'POST', json.encode({username = "ev | House Bought Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)