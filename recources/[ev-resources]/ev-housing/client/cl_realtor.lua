...
RegisterNetEvent("ev-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("ev-housing:getGarages", function()
...
function getIsPropLock()
    ...
    local pp =  RPC.execute("ev-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('ev-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = RPC.execute("ev-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('ev-phone:editingHouse',id,street,price)
    else
        TriggerEvent('ev-phone:sentHouse',nil,id,street,price,category)
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("ev-editor:destroyName", id)
    end
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["ev-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
AddEventHandler("ev-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
AddEventHandler("ev-housing:sellTo", function(pParams)
	...
	if input["cid"] then
		...
		TriggerServerEvent("ev-housing:sell", cid, pParams.propertyId)
	end
end)

RegisterNetEvent("ev-housing:buy")
AddEventHandler("ev-housing:buy", function(pPropertyId, pSeller)
...
RegisterCommand('rent', function()
    TriggerEvent('ev-housing:rent')
end)

RegisterNetEvent("ev-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
RegisterNetEvent("ev-housing:rent2", function(pParams)
    local complete, info = RPC.execute("ev-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
RegisterNetEvent("ev-housing:buyed", function(pParams)
    print('hello?')
    local complete, info = RPC.execute("ev-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
RegisterNetEvent("ev-housing:edit", function()
end)

RegisterNetEvent("ev-housing:editOptions", function(pParams)
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["ev-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:internalBackdoor", function()
    ...
    if max.canHaveBackDoor then
        ...
        DoScreenFadeOut(1)
        exports["ev-build"]:getModule("func").exitCurrentRoom(Housing.currentHousingInteractions.backdoor_coordinates.external)
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["ev-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("ev-housing:blips")
AddEventHandler("ev-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = RPC.execute("ev-housing:owned")
...
RegisterNetEvent('ev-housing:logShit')
AddEventHandler('ev-housing:logShit', function(hID, Price)
    TriggerServerEvent('ev-housing:buyLog', hID, Price)
end)