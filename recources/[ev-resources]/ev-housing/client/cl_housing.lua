...
function Housing.func.enterBuilding(propertyID,enterOverride,counterPart)
    ...
    local isBuiltCoords, objects = exports["ev-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true)
    if counterPart and Housing.staticObjectRobPoints == nil then
        Housing.staticObjectRobPoints = exports["ev-build"]:getModule("func").getRobLocationsForObjects(model,spawnBuildingLocation,Housing.housingRobTargets.static)
    ...
    if isBuiltCoords then
        ...
        TriggerServerEvent("ev-housing:enterHouse", propertyID)

        if not counterPart then
            TriggerEvent("ev-editor:buildName", propertyID, objects)
        end
    else
        Housing.currentHousingInteractions = nil
        Housing.currentlyInsideBuilding = false
...
RegisterNetEvent("ev-housing:loadHousingClient")
AddEventHandler("ev-housing:loadHousingClient", function()
    gatherPlayerInfo()
end)

RegisterNetEvent("ev-housing:reset")
AddEventHandler("ev-housing:reset", function()
    -- cl_housing
    Housing.currentOwned = {}
    Housing.currentKeys = {}
    Housing.currentlyEditing = false
...
RegisterNetEvent("ev-housing:refresh")
AddEventHandler("ev-housing:refresh", function()
    gatherPlayerInfo()
end)
