...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["ev-taskbar"]:taskBar(math.random(5000, 10000), "Breaking in")
...
function alarm(clearAlarm)
    if Housing.alarm then
        local alarmOffset = exports["ev-build"]:getModule("func").getAlarm()
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["ev-textui"]:showInteraction("[E] Take Object")
        else
            exports["ev-textui"]:showInteraction("[E] Look for some belongings.")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["ev-inventory"]:getCurrentWeight()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["ev-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["ev-taskbar"]:taskBar(15000,"Procurando")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("ev-heists:complete", math.random(30, 50))
...
RegisterNetEvent("ev-weathersync:currentTime", function(pHour, pMinute)
...
RegisterNetEvent("ev-housing:alarmFail")
AddEventHandler("ev-housing:alarmFail", function()
...
RegisterNetEvent("ev-housing:alarmEnter")
AddEventHandler("ev-housing:alarmEnter", function()
    insideGame = true
    local successevrp
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("ev-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("ev-housing:alarm")
AddEventHandler("ev-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["ev-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("ev-housing:beingRobbed", function(pPropertyId)
