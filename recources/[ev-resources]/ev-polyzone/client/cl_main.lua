...
local function addToComboZone(zone)
    if comboZone ~= nil then
        ...
        comboZone = ComboZone:Create({ zone }, { name = "ev-polyzone" })
        comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if leftZones ~= nil then
              for i = 1, #leftZones do
                TriggerEvent("ev-polyzone:exit", leftZones[i].name, leftZones[i].data)
              end
            end
            if enteredZones ~= nil then
              for i = 1, #enteredZones do
                TriggerEvent("ev-polyzone:enter", enteredZones[i].name, enteredZones[i].data, enteredZones[i].center)
              end
            end
        end, 500)
    end
...
RegisterNetEvent("ev-polyzone:createCircleZone")
AddEventHandler("ev-polyzone:createCircleZone", function(name, ...)
  addCircleZone(name, ...)
end)
...
if GetConvar("sv_environment", "live") == "debug" then
  RegisterCommand("ev-polyzone:debug", function (src, args)
