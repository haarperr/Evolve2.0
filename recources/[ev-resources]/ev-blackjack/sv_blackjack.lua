...
function tryTakeChips(source,amount)
    ...
    -- local select = SQL('SELECT * FROM characters WHERE id=@id', {['@id'] = cid})
    -- if select ~= nil then
    --     if select[1].chips >= amount then 
    --     local myChip = getChips(cid)
    --     return true
    --     else
    --         return false
    --     end
...
RPC.register("Blackjack:requestBlackjackTableData", function()
    local source = source
    TriggerClientEvent("Blackjack:sendBlackjackTableData",source,blackjackTables)
    return blackjackTables
end)

RPC.register('ev-casino:useChips',function(BetAmount)
    local src = source
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local select = SQL('SELECT * FROM characters WHERE id=@id', {['@id'] = cid})
    if tonumber(BetAmount) <= user:getCash() then
        local myChip = getChips(cid)
         --   exports.oxmysql:execute("UPDATE characters SET chips = @chips WHERE id = @id", {["chips"] = myChip-BetAmount ,['id'] = cid})
        return true
        else
            return false
    end
...
RPC.register('ev-casino:getCurrentChipCount',function()
    local src = source
    local user = exports["ev-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local bal = user:getCash()
    return bal
end)
...
AddEventHandler("Blackjack:setBlackjackBet",function(gameId,betAmount,chairId)
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source,betAmount) then
                        ...
                        TriggerClientEvent("DoLongHudText",source,"Bet placed: " .. tostring(betAmount) .. " Dollars.", 1)
        ...
        TriggerClientEvent("DoLongHudText",source,"Error betting!", 2)
    end
end)
...
AddEventHandler("Blackjack:standBlackjack",function(gameId,nextCardCount, pStatus, pAmount)
    local source = source
                    ...
                    if not isTableEmpty(blackjackGameData[gameId]) then
                            ...
                            if blackjackTables[chairID] ~= false then
                                ...
                                if blackjackGameData[gameId][source] ~= nil then 
                                    ...
                                    if currentHand < 21 then
                                        TriggerClientEvent("Blackjack:standOrHitPrompt",-1,gameId,chairID,nextCardCount,tableId)                            
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    local user = exports["ev-base"]:getModule("Player"):GetUser(source)
                                                    TriggerClientEvent("DoLongHudText",source,'You lost $ '..blackjackGameData[gameId][source][1] , 2)
                                                    user:removeMoney(blackjackGameData[gameId][source][1])
                                                    -- if lostAmount > 9999 then
                                                    --     TriggerClientEvent('chatMessage', -1, "Diamond Casino", 1, "Someone has LOST " .. tostring(getMoneyStringFormatted(lostAmount)) .. " cash!")
                                                    -- end
                                ...
                                TriggerClientEvent("Blackjack:endStandOrHitPhase",-1,tonumber(chairID),tonumber(tableId)) 
                            end
                        end
                        local randomCard = math.random(1,52)
                        --print("randomDealerCard: " .. tostring(randomCard))
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("DoLongHudText",source,"+"..tostring(potentialWinAmount).." Dollars", 1)
                                                            -- if potentialPushAmount > 10000 then
                                                            --     TriggerClientEvent('chatMessage', -1, "Diamond Casino: Someone has WON " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " cash!")
                                                            -- end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("DoLongHudText",source,"+ 0 Dollars", 1)
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("DoLongHudText",source,"-"..tostring(potentialPushAmount).." Dollars", 2)
                                                            user:removeMoney(blackjackGameData[gameId][source][1])
                                                            -- if potentialPushAmount > 9999 then
                                                            --     TriggerClientEvent('chatMessage', -1, "Diamond Casino: Someone has LOST " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " cash!")
                                                            -- end
                                                        end
                                                    end
                                                end
                        ...
                        for chairID=chairIdInitial,chairIdFinal do
                            if blackjackTables[chairID] ~= false then
                               ...
                               --     TriggerClientEvent("Blackjack:chipsCleanup",-1,tostring(chairID).."Dollars",tableId)
