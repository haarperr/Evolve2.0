...
Citizen.CreateThread(function()
  --Wait(10000)
  dealerPedSeeds = RPC.execute("Blackjack:requestBlackjackTableData")
end)
            ...
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        -- exports["ev-ui"]:showInteraction(
                        --   "[E] Play! " .. dwGetLimitTitleString(),
                        --   background
                        -- )
                        exports["ev-ui"]:showInteraction("[E] Play! " .. dwGetLimitTitleString() , background)
                        uiIntShown = true
                    end
                end
    ...
    while true do
        if closestChair ~= -1 and closestChairDist < 2 then 
            if IsControlJustPressed(0, 38) then
                ...
                elseif canPlayAtCurrentTable then
                    TriggerEvent('DoLongHudText', 'You cannot play at this table..',2)
                end
            end
        end
        CasinoWaitTimer()
...
RegisterUICallback("ev-ui:blackjackHitStand", function(data, cb)
  ...
  if action == "hit" then
    waitingForStandOrHitState = false
    TriggerServerEvent("Blackjack:hitBlackjack",globalGameId,1)
    drawTimerBar = false
    standOrHitThisRound = true
    requestCard()
  elseif action == "double" then
    ...
    TriggerServerEvent("Blackjack:hitBlackjack", globalGameId, 1, true)
    requestCard()
  elseif action == "surrender" then
    dwRefreshChipCount = true
    -- RPC.execute("ev-casino:winChips", currentBetAmount / 2)
    TriggerServerEvent("Blackjack:standBlackjack",globalGameId, 1, true, currentBetAmount)
    declineCard()
  elseif action == "stand" then
    waitingForStandOrHitState = false
    TriggerServerEvent("Blackjack:standBlackjack",globalGameId,1)
    drawTimerBar = false
    standOrHitThisRound = true
    declineCard()
  end
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "ev-ui:blackjackHitStand",
...
RegisterUICallback("ev-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("ev-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("ev-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
function getLocalChairIdFromGlobalChairId(globalChairId) --returns tableID based on chairID
    if tonumber(globalChairId) ~= -1 then 
